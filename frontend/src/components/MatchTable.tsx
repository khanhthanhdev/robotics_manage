...
import { MatchType, MatchStatus } from "@/lib/types";

interface MatchTableProps<TData extends object> {
  data: TData[];
...
export function MatchTable<TData extends object>({
  data,
  columns,
  filterUI,
  loading,
  emptyMessage = "No matches found.",
}: MatchTableProps<TData>) {
  ...
  return (
    <Card className="shadow-none border border-gray-800 bg-gray-900">
      {filterUI && <div className="p-4 border-b border-gray-800">{filterUI}</div>}
          ...
          <TableBody>
            {loading ? (
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow key={row.id} className="hover:bg-gray-800/70 transition">
